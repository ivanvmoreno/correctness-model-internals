...
def load_model(model_family: str, model_size: str, model_type: str, device: str):
    ...
    try:
        ...
        if (
            model_family == "Gemma2"
        ):  # Gemma2 requires bfloat16 precision which is only available on new GPUs
            model = model.to(torch.bfloat16)  # Convert the model to bfloat16 precision
        else:
            model = (
                model.half()
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--model_family",
        default="Llama3",
        help="Model family to use. Options are Llama2, Llama3, Gemma, Gemma2 or Mistral.",
    )
    parser.add_argument(
        "--model_size",
        default="8B",
    ...
    parser.add_argument(
        "--model_type",
        default="base",
        help="Whether to choose base or chat model. Options are base or chatorch.",
    )
    ...
    for dataset in datasets:
        ...
        for idx in range(0, len(statements), 25):
            acts = get_acts(
                statements[idx : idx + 25], tokenizer, model, layers, args.device
            )
            for layer, act in acts.items():
